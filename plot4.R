...
library(gridExtra)
p1<-ggplot(df2,aes(x=as.POSIXct(datetime),y=Global_active_power))+geom_line()+scale_x_datetime(labels=labels_format)+xlab("")+ylab("Global Active Power (kilowatts)")
p2<-ggplot(df2,aes(x=as.POSIXct(datetime),y=Voltage))+geom_line()+scale_x_datetime(labels=labels_format)+xlab("datetime")+ylab("Voltage")
p3<-ggplot(df6,aes(x=as.POSIXct(datetime),y=sm,colour=type))+geom_line()+scale_x_datetime(labels=labels_format)+xlab("")+ylab("Energy sub metering")
p4<-ggplot(df2,aes(x=as.POSIXct(datetime),y=Global_reactive_power))+geom_line()+scale_x_datetime(labels=labels_format)+xlab("datetime")+ylab("Voltage")
grid.arrange(p1, p2, p3,p4, ncol=2)
png(filename="plot4.png",width=480,height = 480,type=c("quartz"))
p1<-ggplot(df2,aes(x=as.POSIXct(datetime),y=Global_active_power))+geom_line()+scale_x_datetime(labels=labels_format)+xlab("")+ylab("Global Active Power (kilowatts)")
p2<-ggplot(df2,aes(x=as.POSIXct(datetime),y=Voltage))+geom_line()+scale_x_datetime(labels=labels_format)+xlab("datetime")+ylab("Voltage")
p3<-ggplot(df6,aes(x=as.POSIXct(datetime),y=sm,colour=type))+geom_line()+scale_x_datetime(labels=labels_format)+xlab("")+ylab("Energy sub metering")
p4<-ggplot(df2,aes(x=as.POSIXct(datetime),y=Global_reactive_power))+geom_line()+scale_x_datetime(labels=labels_format)+xlab("datetime")+ylab("Voltage")
